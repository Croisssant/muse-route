...
Examples:
  ...
  python prompts_hard.py --svr-fields connectivity wall_crossings
    ...
    parser.add_argument('--config-file', type=str, default='hard.json',
                       help='Config filename to load from each layout (default: hard.json)')
...
def process_layout(layout_folder, model_name, difficulty, complexity, pbar=None):
    ...
    if pbar:
        pbar.set_description(f"[Hard] {complexity}/{layout_name}")
...
def main():
    ...
    with tqdm(complexity_layouts, desc="[Hard] Processing layouts", 
